...
def GroupConvTest():
    ...
    kernel = np.random.uniform(-1,1,size=(32,16,3,3)).astype('float32')
    i = torch.from_numpy(np.array(input_data))
    k = torch.from_numpy(np.array(kernel))
    conv = torch.nn.Conv2d(in_channels=32,out_channels=16,kernel_size=(3,3), stride=(1,1), padding=(1,1),groups = 2, bias=False)
    conv.weight.data = k
